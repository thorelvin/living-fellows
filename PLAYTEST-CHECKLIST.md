...
- [ ] Recruit, then dismiss or kill a companion, and re-open More > Support: `Action supervisor` reports 0 leaked, and `Action rollback` reports 0 recovering and 0 quarantined. Repeat over five save/load cycles including a permanent death; the leak count stays 0 and no `Leaked reservation` line appears.
- [ ] Return to the main menu and load again: no orphaned supervisor state is reported, and a retired companion contributes no `Leaked reservation` or `Action quarantine` line.
- [ ] Interrupt a companion mid-work (saw, farm or scavenge) with a real survival urgent (an overrun or a critical wound). The work yields the same tick, the urgent owns the actor on the next ticks, and the companion resumes the same job afterwards; the interrupted job must not end in a permanent `retry_exhausted` and must not fall through to a lower-priority behaviour while the urgent runs.
- [ ] With the private diary in inventory, the right-click Read entry appears after a normal start and disappears after a teardown, since the menu hook is now owned by the bootstrap transaction.
- [ ] Let a bitten companion's crisis auto-resolve to quarantine or exile: the subject carries the outcome out and then returns to ordinary activity instead of staying on "Activity: Infection crisis" forever. Remove the quarantine zone and repeat -- the outcome still completes within the bounded window.
- [ ] More > Base still shows the four outcome buttons after the companions have voted, so their verdict can be overruled; "End crisis and return to normal duty" clears the restriction and the companion accepts base jobs again. An authorized mercy killing offers no override.
- [ ] Feed a companion spoiled food until it is badly sick: Status must not report Knox symptoms, no infection crisis opens, and it never speaks turning last words.
