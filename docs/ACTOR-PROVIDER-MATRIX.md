...
|---|---|---|---|---|
| Owned `SCLauncher` native bridge | The transactional installer adds the owned bridge JAR to the launcher classpath and exposes only `SCBridge` to Kahlua. Living Fellows pins bridge protocol `42.20-isocompanion-7` and validates the exact Build 42.20.4 private-method contract before spawning. | Full native companion components | Supported single-player launch path with fail-closed health and cleanup checks | Production provider |
