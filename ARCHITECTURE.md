...
- `SCInfectionCrisis` owns one crisis per bitten or infected survivor. Everyone
  within `crisisGatherRadius` of the subject participates while the crisis is
  `deliberating`, which is why one bite occupies the whole camp; when the
  deliberation timer expires the companions vote for themselves. Only the subject
  keeps crisis behaviour afterwards, and only until the outcome is *carried out*:
  `outcomeCompletedAt` is what releases it, set when the subject reaches
  quarantine or leaves the base, and set anyway after `crisisOutcomeTimeoutMs`
