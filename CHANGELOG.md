...
- Restricted the "catch up to the player" chunk-unload recovery to following companions. A companion set to stay, guard, or base duty is no longer teleported across the map (which could clear or lose the actor) when the player walks out of range; its record is kept intact so it reloads at its post when the player returns.
